...
- A creator's `avatar`/`banner` fields exist in the `fans.foryour.profile` Lexicon but still aren't settable — Phase 8 built PRIVATE media storage (`packages/media`, S3-compatible), not public AT blob upload to a PDS, which is a different, still-unbuilt mechanism (`com.atproto.repo.uploadBlob`). Only `displayName`/`bio`/`website` are wired up.
...
- A real, authenticated AT record write to a live PDS has still not been re-verified since Phase 2 (interactive user consent can't be automated in this environment) — every Lexicon-publishing route (creators, tiers, posts) is covered by tests using a fake AT-record publisher instead. 235 tests total across the workspace as of this phase. Unlike AT writes, Phase 8's S3-compatible storage integration WAS verified against a real backend (a live MinIO container) — a real presigned-URL PUT, GET (with an exact byte-for-byte match), delete, and a confirmed 404 after delete — since MinIO doesn't have Phase 2's "requires an external account and interactive consent" blocker.
...
Phase 9 — Creator and Subscriber Feeds (see `prompts/full.md`).
