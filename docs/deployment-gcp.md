...
> **Terraform** provisions Redis by default — `redis.tf` creates a
> Memorystore instance (`google_redis_instance`) on the default VPC's
> private range, reachable over the same `PRIVATE_RANGES_ONLY` Cloud Run
> egress as Cloud SQL (no Serverless VPC Access connector needed on a v2
> service). `secrets.tf` wires its `host:port` into `ffans-redis-url`.
> To use an external endpoint instead, set `create_redis = false` and put
> its URL in `redis_url` / `TF_VAR_redis_url`.

### Option A (default): Memorystore, provisioned by Terraform

Nothing to do — `terraform apply` creates `ffans-redis` (BASIC tier, 1 GB,
no AUTH/TLS since it's private-network-only). Size/tier via
`var.redis_memory_size_gb` / `var.redis_tier`. See
[Cost tier B](#tier-b--all-google-fully-managed) for the price (~$35/mo).

### Option B (lower cost): external Upstash

Set `create_redis = false`, create a database at
[upstash.com](https://upstash.com) close to `$REGION`, and put its
`rediss://…` URL in `redis_url`. Free tier covers this workload. The
Terraform precondition requires a non-empty `redis_url` whenever
`create_redis = false`.
...
> resources. Redis is wired from `redis.tf` (or from `redis_url` when
> `create_redis = false`). You only supply `admin_dids`.
