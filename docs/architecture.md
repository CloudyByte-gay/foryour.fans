...
Two specs added after Phase 10 change the storage boundary this document describes. They run after the numbered phases (12–17; slot 11 is vacant — see below); nothing above reflects them yet.

- **[`prompts/creator-owned-pds.md`](../prompts/creator-owned-pds.md)** moves creator content off foryour.fans infrastructure. `fans.foryour.profile` / `tier` / `post`, new `fans.foryour.media` / `accessPolicy` / `serviceConfig`, and all media blobs become creator-PDS-owned; gated post bodies and media are **encrypted** before upload, with foryour.fans brokering payment → entitlement → decryption-key grants. `Creator` / `SubscriptionTier` / `Post` / `PostMedia` / `MediaAsset` and the S3 store are demoted to caches/indexes (`isAuthoritative: false`, `sourceUri`, `indexedAt`). The "PUBLIC posts are the only visibility that ever touches the AT network" and "private media never touches a PDS" rules above are superseded — gated content still never becomes a *plaintext public* record, but it does live (encrypted) on the creator's PDS.
- **[`prompts/bluesky-public-posts.md`](../prompts/bluesky-public-posts.md)** dual-publishes every `PUBLIC` post as both `app.bsky.feed.post` and `fans.foryour.post` on the creator's PDS, linked by AT URI/CID. `packages/discovery` and the feed routes gain merge/dedupe logic so the two records render as one item.
