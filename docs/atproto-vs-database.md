...
|---|---|---|---|
| Creator profile | `dev.creator.profile` | `displayName`, `bio`, `avatar`, `banner`, `website`, `createdAt` | `Creator`/`CreatorProfile` (Phase 4) hold `userId`, `did`, `slug`, `status` — operational/internal fields with no reason to be public AT data |
| Subscription tier | `dev.creator.tier` | `name`, `description`, `monthlyPrice` (integer minor units), `currency`, `sortOrder`, `createdAt` | `SubscriptionTier` (Phase 5) additionally holds `isActive`, internal `id`/`creatorId` foreign keys |
| Public post | `dev.creator.post` | `text`, `embed` (media refs), `labels` (content warnings), `createdAt` | `Post` (Phase 7) holds the actual body for `SUBSCRIBERS`/`TIER`-visibility posts — a paid post's content is **never** written as a `dev.creator.post` record; see "Never accidentally publish subscriber-only content into a normal public AT repository" in `prompts/full.md` PHASE 7 |
| Public media | blobs referenced from `avatar`/`banner`/`dev.creator.embed.images` | — | Private/paid media uses `MediaAsset` + application-controlled S3-compatible storage (Phase 8), never a PDS blob |
...
## Why `dev.creator.tier.monthlyPrice` is an integer, not a decimal
